...
async def onmessage(bot:TelegramClient,ev: NewMessage.Event,loop,ret=False):
    ...
    if '/rename' in text:
    	...
    	name = text.split(' ')[2]
    	listdir = await get_root(username)
    	rnmfile = config.ROOT_PATH + username + '/' + listdir[index]
    	rnmname = config.ROOT_PATH + username + '/' + name
    	os.rename(rnmfile,rnmname)
    	await bot.send_message(ev.chat.id,"📂 Archivo renombrado con exito")
